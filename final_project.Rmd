...
```{r}
protected2 <- protected |> filter(YR_EST < 2009| YR_EST %in% c(NA)) #remove after 1981
...
data_years <- steelhead |> group_by(Population) |> summarize(min_year = min(Brood_year), max_year = max(Brood_year), total_year = length(Brood_year)) #find what years I have data over
...
steelhead_new <- steelhead |> filter(Population %in% list) |> filter(Brood_year >= 2009) |> filter(Brood_year <= 2018)


steelhead_limited <- steelhead_new |>  mutate(year = as.numeric(Brood_year) - 2009)
...
model <- lm(Value~percent_protected + year + year:percent_protected, data = steelhead_limited)
...
model <- felm(Value~year | Population, data = steelhead_all)

summary(lm(Value~year, data = steelhead_all))

summary(felm(Value ~ percent_protected |year, data = steelhead_all))
...
coho_usable <- coho2 |> filter(Population %in% list) |> filter(Brood_year >= 2010) |> filter(Brood_year <= 2017)
...
remove_list <- coho_usable2 |> group_by(Population) |> summarize(min_year = min(Brood_year), max_year = max(Brood_year), total_year = length(Brood_year)) |> 
  filter(max_year != 2017 | min_year != 2010)

remove <- remove_list$Population

coho <- coho_usable2 |> filter(!Population %in% remove)

```

```{r}
coho_final <- coho2 |>  mutate(year = as.numeric(Brood_year) - 2010)
