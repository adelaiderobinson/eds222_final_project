...
new <- steelhead |> group_by(Population) |> summarize(percent_protected = max(percent_protected))

steelhead_summary <- steelhead |>  group_by(Population, Brood_year) |> summarize(Value = round(mean(Value),0))

steelhead_final <- left_join(steelhead_summary, new, by = "Population")
...
coho2 <- coho |>  group_by(Population, Brood_year) |> summarize(Value = mean(Value), percent_protected = mean(percent_protected)) #taking the mean estimation for years where estimate done in multiple ways 
